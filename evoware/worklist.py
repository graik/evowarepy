...
from evoware import dialogs as D
from evoware import plates as P
from evoware import samples as S
...
class Worklist(object):
    """
    Low-level Tecan Evoware worklist generator.
...
class SampleWorklist(Worklist):
    ...
    def distributeSamples(self, reactions, reagentkeys=(), wash=True):
        """
        ...
        Args:
            ...
            wash (bool): include wash / tip change statement after dispense
        """
        keys = reagentkeys or self.getReagentKeys(reactions)
        
        ## operate column-wise for optimal tip/plate handling
        for k in keys:
            for tsample in reactions:
...
class Test(testing.AutoTest):
    ...
    def setUp( self ):
        ...
        self.f_sampleworklist = tempfile.mktemp(suffix=".gwl", 
                                                prefix='test_sampleworklist_')
    
    def tearDown(self):
        """Called after *each* test"""
        if not self.DEBUG:
            F.tryRemove(self.fname, verbose=self.DEBUG)
            F.tryRemove(self.f_sampleworklist, verbose=self.DEBUG)
    ...
    def test_sampleworklist(self):
        ...
        P.index['T01'] = P.Plate('T01', format=P.PlateFormat(384))

        converter = DistributionConverter(P.index, reagents=reagents)

        tsamples = S.SampleList(samples, converter=converter)

        with SampleWorklist(self.f_sampleworklist, reportErrors=True) as wl:
            wl.distributeSamples(tsamples)
        fcontent = open(self.f_sampleworklist).readlines()
